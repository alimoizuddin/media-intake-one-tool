...
REM ============================================================
REM  Audio Extractor  v3.0
REM  Pulls audio out of video. Nothing else.
REM
REM  Usage:
REM    Drag and drop one or more files onto this BAT.
REM    Or from a console:
REM      Audio_Extractor.bat [options] "file1" "file2" ...
REM    Run  Audio_Extractor.bat /?  for the option list.
REM
REM  Requires ffmpeg.exe and ffprobe.exe in PATH, or sitting in
REM  the same folder as this BAT file.
REM ============================================================

set "TOOL_NAME=Audio Extractor"
set "TOOL_VERSION=3.0"
set "EXITCODE=0"
...
REM ---- MP3, for listening and for NotebookLM ----
REM Mono at 96 kbps is ample for speech and keeps a 3 hour lecture
REM near 130 MB, comfortably under NotebookLM's 200 MB per-source
REM cap, so no splitting is needed. For music or stereo material,
REM set MP3_CHANNELS=2 and MP3_BITRATE_K=192.
set "MP3_BITRATE_K=96"
set "MP3_CHANNELS=1"
set "MP3_RATE=44100"

REM If the MP3 still goes over MAX_PART_MB, split copies are also
REM written into an mp3_parts subfolder. The full MP3 is kept.
set "MAX_PART_MB=199"
set "SPLIT_SAFETY_PERCENT=95"

REM ---- Transcription audio, for Whisper / Colab / your pipeline ----
REM flac (lossless, smaller) or wav (bigger, universally accepted).
REM Note: NotebookLM does not accept FLAC. Send it the MP3.
...
if /I "%~1"=="/stereo"   ( set "MP3_CHANNELS=2" & set "MP3_BITRATE_K=192" & shift & goto :PARSE )
if /I "%~1"=="/kbps"     ( set "MP3_BITRATE_K=%~2" & shift & shift & goto :PARSE )
...
echo   Or drag and drop files onto the BAT.
echo.
echo Per input file, into OUT_ROOT\yyyy-mm-dd\filename\ :
echo   name.mp3                audio to listen to or send to      (on)
echo                           NotebookLM
echo   name__16k_mono.flac     audio for your transcription       (on)
echo                           pipeline
echo   name__audio_copy.EXT    bit-perfect untouched audio       (off)
...
echo   /copy               also write a bit-perfect audio copy
echo   /stereo             MP3 in stereo at 192 kbps, for music
echo   /kbps N             MP3 bitrate, default %MP3_BITRATE_K%
echo   /maxmb N            size cap before MP3 parts are made
...
REM Divide first. bytes*8*95 overflows the 32-bit range of set /a
REM and silently returns a junk duration.
...
if exist "!FULLPATH!\" (
  echo.
  echo [SKIP] That is a folder, not a file:
  echo        "!FULLPATH!"
  exit /b 2
)
...
if "!HAS_AUDIO!"=="0" (
  echo [SKIP] No audio track in this file. Nothing to extract.
  exit /b 2
)
...
if "!MAKE_TRANSCRIBE!"=="1"    call :MakeTranscribeAudio
if "!MAKE_AUDIO_ARCHIVE!"=="1" call :MakeAudioArchive
...
echo.
echo [1] MP3 audio, !MP3_BITRATE_K! kbps...
>> "!LOG!" echo [mp3] "!MP3_OUT!" bitrate=%MP3_BITRATE% channels=%MP3_CHANNELS%
...
if "!MP3_TOO_BIG!"=="0" (
  echo     [OK] Under !MAX_PART_MB! MB. Ready for NotebookLM as one file.
  exit /b 0
)
...
echo     [INFO] Over !MAX_PART_MB! MB. Writing parts of !SEGMENT_SECONDS! sec...
...
call :IsPlainNumber "%MP3_BITRATE_K%"
if errorlevel 1 (
  echo [ERROR] /kbps needs a whole number, no leading zeros.
  exit /b 1
)
...
if %MP3_BITRATE_K% LSS 32 (
  echo [ERROR] /kbps must be 32 or more.
  exit /b 1
)
if %MP3_BITRATE_K% GTR 320 (
  echo [ERROR] /kbps must be 320 or less.
  exit /b 1
)
exit /b 0
...
set "OUT_ROOT=%~dp0Extracted_Audio"
call :TryOutputRoot "%OUT_ROOT%"
if not errorlevel 1 exit /b 0
